...
#BSUB -J blat-rna
#BSUB -G team274
#BSUB -o /lustre/scratch126/casm/team274sb/lr26/outputs/%J-rnablat.out
#BSUB -e /lustre/scratch126/casm/team274sb/lr26/errors/%J-rnablat.err

### this is an optional script for the BLAT seach of the breakpoints of interest around the cic exon 21 region ###
...
### therefore, define the bam file and region of interest (here it is start, end = 45114510, 45114610 in the Python script)
...
run_blat() {
    ...
    ### the top 2 psl are then written into the pre-prepared csv file which already has the header prepared ###
    tail -n +6 "$psl_file" | LC_ALL=C sort -k1,1nr | head -2 | while IFS=$'\t' read -r matches misMatches repMatches nCount qNumInsert qBaseInsert tNumInsert tBaseInsert strand qName qSize qStart qEnd tName tSize tStart tEnd blockCount blockSizes qStarts tStarts; do
